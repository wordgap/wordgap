...
public class ServerCommunicator {
	...
	/**
	 * Beispiel für JSON String: [["Chapter 8 I soon learned to",
	 * "this flower better. On the little prince's planet the flowers had always been very simple."
	 * , "know", ["move", "live", "rumple"]], ...
	 * 
	 * @param json
	...
	public ArrayList<Sent> parseJSONEx(String json) {
        ...
        final ArrayList<Sent> ex = new ArrayList<>();
        final int noSents = jsonArray.length();
        for(int i = 0; i < noSents; i++) {
            try {
                final JSONArray jsonSent = jsonArray.getJSONArray(i);
                ...
                final JSONArray jsonDis = (JSONArray) jsonSent.get(3);
	...
	public ArrayList<String> parseJSONWordlist(String json) {
        ...
        for(int i = 0; i < jsonArray.length(); i++) {
            try {
                final JSONArray jsonEntry = jsonArray.getJSONArray(i);
                final String lemma = jsonEntry.getString(0);
                final String definition = jsonEntry.getString(1);
                wordlist.add(lemma + "\n\n" + definition);
            }
            catch(final JSONException e) {
	...
	public String getJSONWordlist(ArrayList<String> markedWordsList, String pos)
			throws IOException {

		urlWordlist = ipaddress + "/wordlist/";
		final HttpPost httpPost = new HttpPost(urlWordlist);
		...
		final List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(
				2);
		nameValuePairs.add(new BasicNameValuePair("wordlist", builder
				.toString()));
	 ...
	 * 
	 * @param text
	 *            text that the user wants to be transformed into a
	 *            multiple-choice cloze exercise
	 * @param pos
	 *            requested part-of-speech tag, i.e. nouns, verbs, adjectives or
	 *            prepositions
	 * @return
	 * @throws IOException
	...
	// wird im Moment nicht gebraucht, ist implementiert worden, bevor die
	// WebExAct.
	...
	public String getRawHTML(String url) throws IOException,
		...
		try {
			...
			if (status.getStatusCode() != 200) {
				throw new ServerErrorException("Invalid response from server: "
						+ status.toString());
			}
			final HttpEntity entity = response.getEntity();
